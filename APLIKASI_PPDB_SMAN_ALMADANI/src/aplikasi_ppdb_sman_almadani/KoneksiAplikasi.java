...
public class KoneksiAplikasi {
    ...
    public Connection connect() {
        try {
            ...
            Logger.getLogger(KoneksiAplikasi.class.getName()).log(Level.SEVERE, null, ex);
            System.out.println("Gagal Memuat Ke Drivernya " + ex);
        }
        //untuk koneksi database
        try {
            String url = "jdbc:mysql://localhost:3306/pbo2_java_ppdb";//nama database yang akan dibuat
            koneksi = (Connection) DriverManager.getConnection(url, "root", "");
            System.out.println("Berhasil Terkoneksi Ke Database Anda");
        } catch (SQLException ex) {
            Logger.getLogger(KoneksiAplikasi.class.getName()).log(Level.SEVERE, null, ex);
            System.out.println("Gagal Terkoneksi Ke Database Anda");
